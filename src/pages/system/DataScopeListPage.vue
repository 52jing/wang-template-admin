...
// 资源名称
const label = i18n.t('system.dataScope.resourceTitle')
// 资源地址
const resource = '/system/data_scope'
// 权限前缀
const permPrefix = 'system:data_scope'
// 表格列定义
const columns: M.ResourceTableColumn[] = [
  { name: 'id', required: true, align: 'left', label: 'ID', field: 'id', sortable: true, sortOrder: 'ad' },
  {
    name: 'name',
    label: i18n.t('system.dataScope.name'),
    align: 'left',
    field: 'name',
    sortable: true
  },
  { name: 'remark', label: i18n.t('pages.remark'), align: 'left', field: 'remark' },
  { name: 'operation', required: true, align: 'left', label: i18n.t('pages.Operation'), field: '' }
]
// 详情表单字段定义
const detailConfig: M.DetailConfigProps[] = [
  { name: 'id', label: 'ID' },
  { name: 'name', label: i18n.t('system.dataScope.name') },
...
const formConfig: M.FormConfigProps[] = [
  {
    name: 'name', label: i18n.t('system.dataScope.name'), type: 'text',
    rules: [
      val => val && val.length > 0 || i18n.t('system.dataScope.inputName'),
